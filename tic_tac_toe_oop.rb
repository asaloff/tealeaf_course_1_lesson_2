...
class Board
  ...
  def empty_squares
    @data.select { |_, value| value.empty?}.values
  end

  def empty_positions
    @data.select { |_, value| value.empty?}.keys
  end
  ...
  def all_squares_marked?
    return true if empty_squares.size == 0
  end
end
...
class Game
  ...
  def current_player_marks_square
    if @current_player == @human
      begin
        puts "Please pick a square: #{@board.empty_positions}"
